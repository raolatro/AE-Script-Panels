(function(){
  ...
  var loadMarkersBtn = layerSelectGrp.add("button", undefined, "Load Markers");
  ...
  var explainer = bottomGrp.add("statictext", undefined, "Panel updates text layers, applies animation expressions based on marker durations, and allows marker editing.");
  ...
  animTypeDD.onChange = function(){
    if(animTypeDD.selection.text === "Type"){
      ...
      alert("Animation type set to Typewriter. 'Type By' options shown.");
    } else {
      typeByGrp.visible = false;
  ...
  // Populate Text Layer dropdown with layers that have Source Text property
  ...
  // "Apply Animation" button: apply animation expressions to selected layers
  animateBtn.onClick = function(){
    ...
    for(var i = 0; i < layers.length; i++){
      ...
      // Ensure each marker has at least 2*durVal duration and color yellow
  ...
  //────────────────────────────
  // Edit Markers Section functions
  loadMarkersBtn.onClick = function(){
    app.beginUndoGroup("Load Markers");
    ...
    app.endUndoGroup();
    alert("Markers loaded from layer: " + selLayer.name);
  };
  ...
  markerList.onChange = function(){
    if(markerList.selection){
      ...
      alert("Marker selected. Current text: " + markerEdit.text);
    }
  };

  saveMarkerBtn.onClick = function(){
    app.beginUndoGroup("Save Marker");
    var comp = app.project.activeItem;
    if(!(comp && comp instanceof CompItem)){
      alert("Please select or open a composition.");
      return;
    ...
    markerList.selection.text = formatTime(markerProp.keyTime(index), comp.frameRate) + " : " + marker.comment;
    app.endUndoGroup();
    alert("Marker " + index + " updated with new text: " + marker.comment);
  };

  //────────────────────────────
  // "?" Info button: Show detailed explanation
  infoBtn.onClick = function(){
    ...
    var topPara = infoWin.add("statictext", undefined, "This panel allows you to update Source Text expressions based on markers, apply animation expressions to properties using marker durations, and edit marker comments from a selected text layer.", {multiline:true});
    topPara.maximumSize = [300, 60];
    
    var steps = infoWin.add("statictext", undefined, "Steps:\n1. Use 'Update Expressions' to update text layer expressions (based on selected layers).\n2. In the Animation section, select 'Animated', choose a preset, set durations/easing, then click 'Apply Animation'.\n3. In Edit Markers, select a text layer from the dropdown and click 'Load Markers'.\n4. Edit a marker’s text in the field and click 'Save Marker'.", {multiline:true});
